...
class EDM_Style_Multitrack(EDM):
    ...
    def prepare_Fxenc(self, FXenc_args, *args, **kwargs):
        if FXenc_args.type=="AFxRep":
            ...
            #Now prepare the adaptor
            fxencAF_adaptor_args=kwargs.get("fxencAF_adaptor_args", None)
            from utils.load_features import load_fxencAF_adaptor
            self.FXenc_adaptor=load_fxencAF_adaptor(fxencAF_adaptor_args, self.device)
    ...
    def loss_fn(self, net,  sample=None, context=None, clusters=None, taxonomy=None, masks=None, compile=False,*args, **kwargs):
        ...
        with torch.no_grad():
            ...
            y_style=self.style_encode(y, compile=compile, masks=masks, taxonomy=taxonomy, use_adaptor=False)
            context_style=self.style_encode(context, compile=compile, masks=masks, taxonomy=taxonomy, use_adaptor=True)
            ...
            input, target, cnoise, input_context = self.prepare_train_preconditioning(y_style, t, context_style )
        ...
        estimate = net(input, cnoise, cross_attn_cond=z, taxonomy=taxonomy, mask=masks, cross_attn_cond_mask=masks, input_concat_cond=input_context)  #this will crash because of broadcasting problems, debug later!
    ...
    def denoiser(self, xn , net, t, cond=None,cfg_scale=1.0, masks=None, taxonomy=None, **kwargs):
        ...
        cond_CLAP_emb, cond_style_emb = cond

        cond_style_emb=cin*cond_style_emb

        if cfg_scale == 1.0:
            net_out=net(x_in, cnoise.to(torch.float32), cross_attn_cond=cond_CLAP_emb, mask=masks, taxonomy=taxonomy,  cross_attn_cond_mask=masks, input_concat_cond=cond_style_emb)  #this will crash because of broadcasting problems, debug later!
        else:
    ...
    def style_encode(self, x, compile=False, masks=None, taxonomy=None, use_adaptor=False):
        ...
        def apply_fxenc(x_masked, taxonommy=None):
            ...
            if use_adaptor:
                x_emb=self.FXenc_reshape(x_emb)
                x_emb=self.FXenc_adaptor(x_emb)
                x_emb=x_emb.view(x_emb.shape[0], 64, -1)  # Reshape to [B, 64, L//64] where L//64 is the number of frames
