...
n="tencymastering_multitrack_simulated_styleAFxRep_contentCLAP_fxnorm_CLAPadaptor_smallnoise"
...
python train_sim_multitrack.py --config-name=$conf  \
  model_dir=$PATH_EXPERIMENT \
  diff_params=edm_style_AFxRep_CLAP_simulated_multitrack_fxnorm_CLAPadaptor_smallnoise.yaml \
  exp.resume=True \
  exp.compile=False \
