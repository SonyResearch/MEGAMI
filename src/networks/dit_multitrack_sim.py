        ...
        **kwargs):
        ...
        if pos_emb_strategy == "concatenation":
            ...
            if pos_emb_type == "one-hot":
                ...
                self.pos_emb = OneHotPositionalEmbedding(pos_emb_dim-3)
                ...
                def concat_pos_emb(x, pos=None, seq_start_pos=None, taxonomy=None):
                    ...
                    assert pos_emb.shape[-1] == pos_emb_dim-3, f"pos_emb shape mismatch: {pos_emb.shape[-1]} != {pos_emb_dim}"
                    ...
                    pos_emb_track=torch.zeros((B, N, T, 3), device=x.device, dtype=x.dtype)
                    for i in range(B):
                        for j in range(N):
                            if use_taxonomy_in_pos_emb:
                                ...
                                pos_emb_track[i, j, :, :]= F.one_hot(torch.tensor([j], device=x.device), num_classes=3).to(x.dtype).expand(T, -1)
        ...
        if pos_emb_crossattn_strategy == "concatenation":
            ...
            if pos_emb_crossattn_type == "one-hot":
                ...
                self.pos_emb_crossattn = OneHotPositionalEmbedding(pos_emb_crossattn_dim-3)
