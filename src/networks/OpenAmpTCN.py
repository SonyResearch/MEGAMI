...
class TCN(nn.Module):
    def __init__(self, 
        ...
        if self.use_CLAP:
            ...
            self.emb_dim=cond_dim
        


        self.first_conv = nn.Conv1d(in_channels=ninputs, out_channels=channels, kernel_size=1)
        for b in range(blocks):
            self.blocks.append(TCNBlock( channels, channels, dilation_growth, kernel_size, layers, self.emb_dim))
        self.last_conv = nn.Conv1d(in_channels=channels, out_channels=noutputs, kernel_size=1)
    ...
    def forward(self, x, c):
        ...
        if self.use_CLAP:
            with torch.no_grad():
                c = self.merge_CLAP_embeddings(x, c)
...
class TCNBlock(nn.Module):
    def __init__(self,  chan_input, chan_output, dilation_growth, kernel_size, layers, emb_dim):
        super(TCNBlock, self).__init__()
        self.channels = chan_output
        dilations = [dilation_growth ** lay for lay in range(layers)]
        self.layers = nn.ModuleList()
        for dil in dilations:
            self.layers.append(GatedTCNLayerFilmConditioned(chan_input, chan_output, dil, kernel_size, emb_dim))
...
class GatedTCNLayerFilmConditioned(nn.Module):
    ...
    def __init__(self, chan_input, chan_output, dilation, kernel_size, emb_dim):
        super().__init__()
        self.channels = chan_output
        self.in_chan = chan_input
        ...
        self.conv = nn.Conv1d(in_channels=chan_input, out_channels=chan_output*2, kernel_size=kernel_size, stride=1,
                              padding=0, dilation=dilation)
        self.film = FiLM(num_features=chan_output*2, cond_dim=emb_dim)
