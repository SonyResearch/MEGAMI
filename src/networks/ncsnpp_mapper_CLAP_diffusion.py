...
ResnetBlockBigGANFiLM = layerspp.ResnetBlockBigGANppFiLM
...
class NCSNpp(nn.Module):
        ...
        **kwargs):
        ...
        if resblock_type == 'ddpm':
            ...
            ResnetBlock = functools.partial(ResnetBlockBigGAN, act=act,
                ...
                init_scale=init_scale, skip_rescale=skip_rescale, temb_dim=(nf * 16 if time_conditional else None))
        elif resblock_type == 'bigganfilm':
            ResnetBlock = functools.partial(ResnetBlockBigGANFiLM, act=act,
                dropout=dropout, fir=fir, fir_kernel=fir_kernel, 
                init_scale=init_scale, skip_rescale=skip_rescale, temb_dim=(nf * 16 if time_conditional else None))
        else:
        ...
        modules.append(nn.Linear(embed_dim, nf * 16))
        modules[-1].weight.data = default_initializer()(modules[-1].weight.shape)
        nn.init.zeros_(modules[-1].bias)
        modules.append(nn.Linear(nf * 16, nf * 16))
        modules[-1].weight.data = default_initializer()(modules[-1].weight.shape)
        nn.init.zeros_(modules[-1].bias)
    ...
    def forward(self, x, cond=None, time_cond=None):
        ...
        assert time_cond is not None, "time_cond must be provided for NCSNpp"
        assert cond is not None, "cond must be provided for NCSNpp"
        ...
        print("temb.shape", temb.shape, "cond.shape", cond.shape)
        temb=torch.cat((temb, cond.view(cond.shape[0],-1)), dim=-1)  # b,2*nf
...
class NCSNppTimeFxModel(NCSNpp):
    ...
    def __init__(self, stft=None,
        ...
        **kwargs)
        ...
        self.use_context_phases = use_context_phases
        ...
        if self.use_CLAP:
            ...
            def merge_CLAP_embeddings(x, emb):
                ...
                #print("clap_embedding.shape", clap_embedding.shape, "emb.shape", emb.shape  )
    ...
    def forward(self, x,  cond, time_cond=None, input_concat_cond=None):
        ...
        if input_concat_cond is not None:
            ...
            assert input_concat_cond.shape[-1]==x.shape[-1], "input_concat_cond must have the same shape as x {} but got {}".format(input_concat_cond.shape, x.shape)
            input_stft=torch.cat([x, input_concat_cond], dim=1)
            res_stft=self.stft(input_stft)

            x_spec=res_stft[:, :C]  # b,2*D,F,T
            if not self.use_context_phases:
                cond_spec=res_stft[:, C:].abs()
            else:
                cond_spec=res_stft[:, C:]
        else:
            x_spec=self.stft(x)
        ...
        if input_concat_cond is not None:
            if not self.use_context_phases:
                ...
                x_spec_in = torch.cat([x_spec_in, cond_spec.real, cond_spec.imag], dim=1)

        x_spec=super().forward(x_spec_in, cond=cond, time_cond=time_cond)
