...
class ContinuousTransformer(nn.Module):
        ...
        **kwargs
        ...
        for i in range(depth):
            ...
            self.layers.append(
                TransformerBlock(
                    dim+self.extra_dim,
                    dim_heads = (dim+self.extra_dim) // num_heads,
                    cross_attend = should_cross_attend,
                    dim_context = cond_token_dim + self.crossattn_extra_dim,
    ...
    def forward(
        ...
        **kwargs
        ...
        x= self.pos_emb_fn(x)
        context= self.pos_emb_crossattn_fn(context) if context is not None else None
        ...
        for layer in self.layers:
            ...
            x = checkpoint(layer, x, rotary_pos_emb = None, global_cond=global_cond, context=context, **kwargs)

            if return_info:
                info["hidden_states"].append(x)

        x=self.remove_pos_emb(x)
