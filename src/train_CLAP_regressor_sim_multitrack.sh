...
# main config
conf=conf_CLAP_regressor_sim_multitrack.yaml

n="CLAP_regressor_sim_multitrack"
...
python train_CLAP_regressor_sim_multitrack.py --config-name=$conf  \
  model_dir=$PATH_EXPERIMENT \
  exp.resume=True \
  dset.validation.num_tracks=-1 \
  dset.validation_2.num_tracks=35 \
