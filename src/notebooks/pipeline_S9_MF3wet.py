...
if args.exp.apply_fxnorm:
    ...
    fx_normalizer= lambda x: x  # identity function if no fx_normalizer is specified
...
def generate_Fx(x, input_type="dry",num_samples=1, T=50, Schurn=10, cfg_scale=1.0):
    ...
    masks_fwd= torch.ones((B, N), dtype=torch.bool, device=device)  # Create masks for all tracks, assuming all tracks are present
    masks_diff= torch.ones((num_samples, N), dtype=torch.bool, device=device)  # Create masks for all tracks, assuming all tracks are present

    sampler.Schurn=Schurn  # Set the Schurn parameter for the sampler

    sampler.T=T

    with torch.no_grad():
        is_wet= "wet" in input_type
        cond, x_preprocessed=sampler.diff_params.transform_forward(x,  is_condition=True, is_test=True, masks=masks_fwd, is_wet=True)
        cond=cond.expand(shape[0], -1, -1,-1)  # Expand the condition to match the batch size
        preds, noise_init = sampler.predict_conditional(shape, cond=cond.contiguous(), cfg_scale=cfg_scale, device=device,  masks=masks_diff)
...
def apply_effects(x, z_pred):
    ...
    x_norm=fx_normalizer(x_norm)  # Apply the fx_normalizer if specified
...
for i in range(len(dataset_val)):
    ...
    #save audio file
    sf.write(f"{path_name}/ref_{i}.wav", mixture[0].cpu().clamp(-1,1).numpy().T, 44100, subtype='PCM_16')
    ...
    sf.write(f"{path_name}/ref_blackbox_{i}.wav", y_hat_mixture.cpu().clamp(-1,1).numpy().T, 44100, subtype='PCM_16')  # Save the generated audio
    #display(a)
    ...
    for j in range(z_pred.shape[0]):
        ...
        sf.write(f"{path_name}/pred_blackbox_{i}_{j}.wav", y_hat_mixture.cpu().clamp(-1,1).numpy().T, 44100, subtype='PCM_16')


    preds=generate_Fx(x, input_type="dry", num_samples=100) 
    ...
    fig.savefig(f"{path_name}/tsne_blackbox_{i}.png")
