...
config_name="conf_MF3_mapper_blackbox_predictive_fxenc2048AFv3CLAP_paired.yaml"
model_dir="/data5/eloi/checkpoints/MF3"
ckpt="mapper_blackbox_TCN-80000.pt"
...
os.makedirs("examples_MF3_S4", exist_ok=True)  # Create the directory to save the examples
for i in range(40):
    datav=dataset_val[i],
    ...
    #save audio file
    sf.write(f"examples_MF3_S4/ref_{i}.wav", mixture[0].cpu().clamp(-1,1).numpy().T, 44100, subtype='PCM_16')
    ...
    sf.write(f"examples_MF3_S4/ref_blackbox_{i}.wav", y_hat_mixture.cpu().clamp(-1,1).numpy().T, 44100, subtype='PCM_16')  # Save the generated audio
    #display(a)
    ...
    for j in range(z_pred.shape[0]):
        ...
        sf.write(f"examples_MF3_S4/pred_blackbox_{i}_{j}.wav", y_hat_mixture.cpu().clamp(-1,1).numpy().T, 44100, subtype='PCM_16')
    ...
    fig.savefig(f"examples_MF3_S4/tsne_blackbox_{i}.png")
