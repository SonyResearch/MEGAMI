...
ckpt="1C_tencymastering_vocals-60000.pt"
...
import os
os.makedirs("examples_S3_train_3", exist_ok=True)  # Create the directory to save the examples
...
for i in range(len(dataset_val)):
    ...
    #save audio file
    sf.write(f"examples_S3_train_2/ref_{i}.wav", mixture[0].cpu().clamp(-1,1).numpy().T, 44100, subtype='PCM_16')
    ...
    sf.write(f"examples_S3_train_2/ref_blackbox_{i}.wav", y_hat_mixture.cpu().clamp(-1,1).numpy().T, 44100, subtype='PCM_16')  # Save the generated audio
    #display(a)
    ...
    for j in range(z_pred.shape[0]):
        ...
        sf.write(f"examples_S3_train_2/pred_blackbox_{i}_{j}.wav", y_hat_mixture.cpu().clamp(-1,1).numpy().T, 44100, subtype='PCM_16')
    ...
    fig.savefig(f"examples_S3_train_2/tsne_blackbox_{i}.png")
