...
def _main(args):
    ...
    from testing.tester_mapper_diffusion import Tester
    import copy
    network_tester = copy.deepcopy(network).eval().requires_grad_(False)
    tester = Tester(args, network_tester,  device=device, in_training=True, test_set_dict=val_set_dict, diff_params=diff_params)
    ...
    from training.trainer_mapper_blackbox_diffusion import Trainer
    #print("args", args)
    trainer = Trainer( args=args, dset=train_loader, network=network, device=device, tester=tester, distributed=False, diff_params=diff_params)  # This works
