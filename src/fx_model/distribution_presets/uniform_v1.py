...
def get_distributions_uniform(sample_rate: int = 44100):
    ...
    distributions_PEQ = {
        "peak1_gain": Uniform(low=-20.0, high=20, shape=(1,)),  # Lower gain for a more subtle effect
        "peak1_freq": LogUniform(low=33, high=5400, shape=(1,), transformation=lambda x: torch.clamp(x, min=33, max=5400)),  # Focus on lower mids
        "peak1_Q": Uniform(low=0.51, high=9.99, shape=(1,), transformation=lambda x: torch.clamp(x, min=0.5, max=10)),  # Slightly broader Q for warmth
    
        "peak2_gain": Uniform(low=-20.0, high=20.0, shape=(1,)),  # Lower gain for a more natural sound
        ...
        "peak2_Q": Uniform(low=0.51, high=9.99, shape=(1,), transformation=lambda x: torch.clamp(x, min=0.2, max=10)),  # Standard Q for balance
    
        "lowshelf_gain": Uniform(low=-20.0, high=20, shape=(1,)),  # Less aggressive cut for lows
        ...
        "highshelf_gain": Uniform(low=-20.0, high=20, shape=(1,)),  # Moderate boost for highs
        ...
        "lowpass_freq": LogUniform(low=10001, high=17999, shape=(1,), transformation=lambda x: torch.clamp(x, min=10001, max=18000)),  # Slightly lower cutoff for a warmer sound
        "lowpass_Q": Uniform(low=0.51, high=9.9, shape=(1,), transformation=lambda x: torch.clamp(x, min=0.5, max=10)),  # Standard Q for balance
    
        "highpass_freq": LogUniform(low=17, high=499, shape=(1,), transformation=lambda x: torch.clamp(x, min=16, max=500)),  # Lower cutoff for retaining warmth
    ...
    distributions_CompExp = {
        ...
        "exp_ratio": Uniform(low=0.20, high=0.99, shape=(1,), transformation=lambda x: torch.clamp(x, min=0.001, max=0.999)),
    ...
    distributions_FDN = {
        "b": Normal(mean=0, std=0.5, shape=(6,2), transformation=lambda x: torch.clamp(x, min=-1 , max= 1  )),  # in milliseconds (use uniform distribution in +- 1/sqrt(6) range)
        "c": Normal(mean=0, std=0.5, shape=(2,6),  transformation=lambda x: torch.clamp(x, min=-1 , max= 1  )),  # in milliseconds
        "U": Normal(mean=0, std=0.5, shape=(6,6), transformation= lambda x:unilossless(x.clamp(-1,1)) ),  # in milliseconds
        "gamma": UniformRamp(low_low=0.01, low_high=0.01, high_low=2, high_high=1, log=True, shape=(49, 1), transformation= lambda x: T60togamma(torch.clamp(x, min=0, max=T60_max), sample_rate=sample_rate)),  # decay frequencies
    }
    ...
    distributions_PEQ_FDN = {
        ...
        "peak1_Q": Uniform(low=0.51, high=2.99, shape=(1,), transformation=lambda x: torch.clamp(x, min=0.5, max=3)),
        ...
        "peak2_Q": Uniform(low=0.51, high=2.99, shape=(1,), transformation=lambda x: torch.clamp(x, min=0.5, max=3)),
    ...
    distributions_RMSnorm = {
        "RMSnorm": Uniform(low=-40, high=-10, shape=(1,),transformation=lambda x: torch.clamp(x, -40, -10) ),  # Panning parameter between 0 and 1
    }
