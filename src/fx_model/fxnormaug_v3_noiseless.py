...
class FxNormAug:
    ...
    def inference_setup(self):
        ...
        self.distribution_delta_log_magnitude_1 = NormalRamp(mean_low=1, mean_high=0.05, std_low=0.01, std_high=0.01, log=True, shape=(self.reverberator.num_bins,), transformation=lambda x: T602logmag(x.clamp(min=0.001), sample_rate=self.sample_rate, hop_length=self.reverberator.hop_length))  # Range for delta log magnitude randomization

        self.distribution_drywet_ratio_1= Normal(mean=0.5, std=0, shape=(1,), transformation=lambda x: torch.clamp(x, min=0, max=1))  # Dry/wet ratio for reverb
    ...
    def train_setup(self):
        ...
        self.distribution_init_log_magnitude_1=Normal(mean=0, std=1, shape=(self.reverberator.num_bins,))  # Range for log magnitude randomization
        self.distribution_delta_log_magnitude_1 = NormalRamp(mean_low=0.8, mean_high=0.02, std_low=0.3, std_high=0.05, log=True, shape=(self.reverberator.num_bins,), transformation=lambda x: T602logmag(x.clamp(min=0.001), sample_rate=self.sample_rate, hop_length=self.reverberator.hop_length))  # Range for delta log magnitude randomization

        self.distribution_drywet_ratio_1= Normal(mean=0.6, std=0.15, shape=(1,), transformation=lambda x: torch.clamp(x, min=0, max=1))  # Dry/wet ratio for reverb
        ...
        self.distributions_CompExp_2 = {
            "comp_ratio": Normal(mean=4.0, std=2.0, shape=(1,), transformation=lambda x: torch.clamp(x, min=1.0001)),  # compression ratio
