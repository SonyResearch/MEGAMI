...
class STFTMaskedNoiseReverb_filterbank(nn.Module):
    r"""
    ...
    def __init__(
        ...
        if self.use_filterbank:
            self.filterbank = TriangularFilterBank(
                num_frequency_bins=self.num_bins, **filterbank_kwargs
            )
    ...
    def sample_noise(self, num_noises, device):
        if self.processor_channel=="mono":
            c=1
        else:
            c=2
        noise = torch.rand(num_noises * c, self.ir_len, device=device) * 2 - 1
        ...
        noise_stft = rearrange(noise_stft, "(b c) f t -> b c f t", c=c)


        #STFT magnitudes must have 0dB 
    ...
    def compute_ir(
        ...
        if self.use_filterbank:
            assert gain_env_log_magnitude is None, "gain_env_log_magnitude must be None when using filterbank"

            assert init_log_magnitude.shape == delta_log_magnitude.shape, "init_log_magnitude and delta_log_magnitude must have the same shape"

            shape= init_log_magnitude.shape
            shape, f = shape[:-1], shape[-1]
            init_log_magnitude= init_log_magnitude.view(-1, f)

            init_energy=torch.exp(init_log_magnitude).square()
            self.filterbank.to(init_energy.device)
            init_log_magnitude = self.filterbank(init_energy, mode="synthesis")
            init_log_magnitude = torch.log(torch.sqrt(init_log_magnitude + self.eps))

            init_log_magnitude = rearrange(init_log_magnitude, "(b c) f -> b c f", b=shape[0], c=shape[1])

            shape2= delta_log_magnitude.shape
            shape2, f2 = shape2[:-1], shape2[-1]
            delta_log_magnitude = delta_log_magnitude.view(-1, f2)

            delta_energy = torch.exp(delta_log_magnitude).square()
            delta_log_magnitude = self.filterbank(delta_energy, mode="synthesis")
            delta_log_magnitude = torch.log(torch.sqrt(delta_log_magnitude + self.eps))

            delta_log_magnitude = rearrange(delta_log_magnitude, "(b c) f -> b c f", b=shape2[0], c=shape2[1])

            assert delta_log_magnitude.shape == init_log_magnitude.shape, "delta_log_magnitude and init_log_magnitude must have the same shape"
        ...
        ir = rearrange(ir, "(b c) t -> b c t", c=C)
    ...
    def compute_stft_mask(
        ...
        #delta_log_magnitude=6.908/ (T60*(self.sample_rate/self.hop_length)) 
        #delta_log_magnitude = -F.softplus(delta_log_magnitude)[:, :, :, None]
        delta_log_magnitude = -torch.abs(delta_log_magnitude)[:, :, :, None]
        ...
        mask = torch.exp(mask_log_magnitude)
    ...
    def parameter_size(self):
        ...
        if self.use_filterbank:
            if self.processor_channel == "mono":
                size = {
                    "init_log_magnitude": (1, self.filterbank.num_filters),
                    "delta_log_magnitude": (1, self.filterbank.num_filters),
                }
                if self.gain_envelope:
                    size["gain_env_log_magnitude"] = (1, self.num_frames)
            else:
                size = {
                    "init_log_magnitude": (2, self.filterbank.num_filters),
                    "delta_log_magnitude": (2, self.filterbank.num_filters),
                }
                if self.gain_envelope:
            ...
            if self.processor_channel == "mono":
                size = {
                    "init_log_magnitude": (1, self.num_bins),
                    "delta_log_magnitude": (1, self.num_bins),
                }
                if self.gain_envelope:
                    size["gain_env_log_magnitude"] = (1, self.num_frames)
            else:
...
class STFTEQ(nn.Module):
    ...
    def __init__(
        ...
        self.n_fft = n_fft
        self.hop_length = hop_length
        self.num_frames = 1 + (self.ir_len // hop_length)
        self.num_bins = 1 + n_fft // 2
    ...
    def compute_ir(
        ...
        if self.zero_phase:
            ir_stft= ir_stft.abs().to(dtype=torch.complex64)
    ...
    def compute_stft_mask(
        ...
        mask_log_magnitude = init_log_magnitude + torch.zeros_like(init_log_magnitude) * self.arange
