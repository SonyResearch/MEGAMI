...
def prepare_compexp_parameters(
    ...
    atmin=torchcomp.ms2coef(torch.tensor([0.1]), sample_rate)
    atmax=torchcomp.ms2coef(torch.tensor([300.0]), sample_rate)
    rtmin=torchcomp.ms2coef(torch.tensor([5.0]), sample_rate)
    rtmax=torchcomp.ms2coef(torch.tensor([1500.0]), sample_rate)
    dict_transformations= {
        "comp_ratio": MinMax(1.0, 20.0),
        "exp_ratio": MinMax(0.20, 0.99),
        "at_coef": MinMax(atmin.item(), atmax.item()),  # attack time in ms
        "rt_coef": MinMax(rtmin.item(),rtmax.item()),  # release time in ms
        "avg_coef": MinMax(0.5, 0.995),  # average coefficient
        "comp_th": MinMax(-30,-5),
        "exp_th": MinMax(-50, -15),
        "make_up": MinMax(-6,12),
