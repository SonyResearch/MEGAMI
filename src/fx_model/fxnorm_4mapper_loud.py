...
from utils.data_utils import taxonomy2track, apply_loud_normalization
...
class FxNormAug:
    ...
    def __init__(self,
                ...
                seed=42,
                features_path="/home/eloi/projects/project_mfm_eloi/src/fx_model/features/features_tency1_4instr_v4.npy",  # Path to the features file
                ):
    ...
    def train_setup(self):
        ...
        self.distributions_CompExp_1 = {
            "comp_ratio": Normal(mean=3.0, std=0.0, shape=(1,), transformation=lambda x: torch.clamp(x, min=1.0001)),  # compression ratio
            "exp_ratio": Normal(mean=0.33, std=0.0, shape=(1,), transformation=lambda x: torch.clamp(x, min=0.01, max=0.999)),
    ...
    def EQ_normalize_setup(self ):
        ...
        def EQ_normalize_fn(x, taxonomy=None, random=False):
