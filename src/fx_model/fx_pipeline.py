...
class DiffVoxFx(nn.Module):

    def __init__(self, 
        sample_rate=44100,
        device="cuda" if torch.cuda.is_available() else "cpu",
        ):
        super(DiffVoxFx, self).__init__()
        ...
        #self.gain_param_transformation = MinMax(min=-10, max=10)  # Transformation for gain parameter

        #self.gain_dB_param=torch.nn.Parameter(torch.tensor(0.0), requires_grad=True)  # Initialize gain parameter to 0 dB
    ...
    def forward(self, x):
        ...
        gain_dB= self.gain_param_transformation(self.gain_dB_param).view(-1,1,1)  # Ensure gain is broadcastable
...
class DiffVoxFxMaster(nn.Module):

    def __init__(self, 
        sample_rate=44100,
        device="cuda" if torch.cuda.is_available() else "cpu",
        ):
        super(DiffVoxFxMaster, self).__init__()
        ...
        self.gain_dB_param=torch.nn.Parameter(torch.tensor([0.0, 0.0]), requires_grad=True)  # Initialize gain parameter to 0 dB
    ...
    def forward(self, x):
        ...
        for key, param in self.CompExp_params.items():
            param_transformed= self.CompExp_transformations[key](param)
            batched_params_CompExp[key] = param_transformed.unsqueeze(0).repeat(x.size(0)*x.size(1), *([1] * (param_transformed.dim() )))  # Reshape to (batch_size * num_channels, num_samples)
        
        x=einops.rearrange(x, 'b c t -> (b c) 1 t')  # Reshape to (batch_size, num_channels * num_samples)
        x= compexp_functional(x, **batched_params_CompExp, **self.params_CompExp_non_optimizable)
        x= einops.rearrange(x, '(b c) 1 t -> b c t', b=x.size(0), c=x.size(1))  # Reshape back to (batch_size, num_channels, num_samples)

        #pan_param_transformed= self.pan_param_transformation(self.pan_param)
        #x_pan=panning( x, pan_param_transformed)


        gain_dB= self.gain_param_transformation(self.gain_dB_param).view(-1,2,1)  # Ensure gain is broadcastable
        x= x * (10 ** (gain_dB / 20))  # Apply gain in dB

        return x

class DiffVoxFxMultitrack(nn.Module):

    def __init__(self, 
        sample_rate=44100,
        device="cuda" if torch.cuda.is_available() else "cpu",
        N_tracks=2,  # Number of tracks to process in parallel
        ):
        super(DiffVoxFxMultitrack, self).__init__()

        self.sample_rate = sample_rate
        self.N_tracks = N_tracks

        params_PEQ_optimizable, params_PEQ_non_optimizable, dict_transformations = prepare_PEQ_parameters(sample_rate, device=device)
        self.PEQ_params = nn.ModuleList([
                nn.ParameterDict({
                    key: torch.nn.Parameter(value.clone()) for key, value in params_PEQ_optimizable.items()
        ...
        self.PEQ_FDN_params = nn.ModuleList([
                nn.ParameterDict({
                    key: torch.nn.Parameter(value.clone()) for key, value in params_PEQ_FDN_optimizable.items()
                    }) for _ in range(N_tracks)
            ])
        ...
        self.CompExp_params = nn.ModuleList([
                nn.ParameterDict({
                    key: torch.nn.Parameter(value.clone()) for key, value in params_CompExp_optimizable.items()
                    }) for _ in range(N_tracks)
            ])
        ...
        self.FDN_params = nn.ModuleList([
                nn.ParameterDict({
                    key: torch.nn.Parameter(value.clone(), requires_grad=True) for key, value in params_FDN_optimizable.items()
                    }) for _ in range(N_tracks)
            ])
        #self.FDN_params = nn.ParameterDict({
        #    key: torch.nn.Parameter(value, requires_grad=True) for key, value in params_FDN_optimizable.items()
        ...
        self.pan_param= [
            torch.nn.Parameter(self.pan_param_transformation.right_inverse(pan), requires_grad=True) for _ in range(N_tracks)
        ]
        #torch.nn.Parameter(self.pan_param_transformation.right_inverse(pan), requires_grad=True)
        ...
        self.gain_dB_param= [
            torch.nn.Parameter(torch.tensor(0.0), requires_grad=True) for _ in range(N_tracks)
        ]
    ...
    def forward(self, x):
        ...
        for i in range(self.N_tracks):
            for key, param in self.PEQ_params[i].items():
                param_transformed= self.PEQ_transformations[key](param)
                if i== 0:
                    batched_params_PEQ[key] = param_transformed.unsqueeze(0)
                else:
                    batched_params_PEQ[key] = torch.cat((batched_params_PEQ[key], param_transformed.unsqueeze(0)), dim=0)

            for key, param in self.CompExp_params[i].items():
                param_transformed= self.CompExp_transformations[key](param)
                if i== 0:
                    batched_params_CompExp[key] = param_transformed.unsqueeze(0)
                else:
                    batched_params_CompExp[key] = torch.cat((batched_params_CompExp[key], param_transformed.unsqueeze(0)), dim=0)
            
            for key, param in self.FDN_params[i].items():
                param_transformed= self.FDN_transformations[key](param)
                if i== 0:
                    batched_params_FDN[key] = param_transformed.unsqueeze(0)
                else:
                    batched_params_FDN[key] = torch.cat((batched_params_FDN[key], param_transformed.unsqueeze(0)), dim=0)
            
            for key, param in self.PEQ_FDN_params[i].items():
                param_transformed= self.PEQ_FDN_transformations[key](param)
                if i== 0:
                    batched_params_PEQ_FDN[key] = param_transformed.unsqueeze(0)
                else:
                    batched_params_PEQ_FDN[key] = torch.cat((batched_params_PEQ_FDN[key], param_transformed.unsqueeze(0)), dim=0)
        ...
        pan_param_transformed = torch.zeros((self.N_tracks,1), device=x.device)
        for i in range(self.N_tracks):
            pan_param_transformed[i]= self.pan_param_transformation(self.pan_param[i])
        ...
        gain_dB = torch.zeros((self.N_tracks, 1, 1), device=x.device)
        for i in range(self.N_tracks):
            gain_dB[i]= self.gain_param_transformation(self.gain_dB_param[i]).view(-1,1,1)  # Ensure gain is broadcastable
