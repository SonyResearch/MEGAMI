...
# main config
conf=conf_A5_fxenc_regressor_multitrack.yaml

n="fxenc_regressor_multitrack"
...
python A5_train_fxenc_regressor_multitrack.py --config-name=$conf  \
  model_dir=$PATH_EXPERIMENT \
  exp.resume=True \
  dset.validation.num_tracks=4 \
  dset.validation_2.num_tracks=4 \
  exp.num_workers=4 \
