...
def _main(args):
    ...
    train_loader = torch.utils.data.DataLoader(dataset=train_set, batch_size=args.exp.batch_size,
                                               ...
                                               worker_init_fn=worker_init_fn, timeout=0, prefetch_factor=10)
    ...
    from testing.tester_simulated import Tester
    import copy
    ...
    from training.trainer_mapper_blackbox_predictive import Trainer
    #print("args", args)
    trainer = Trainer( args=args, dset=train_loader, val_set_dict=val_set_dict, network=network, device=device, tester=tester, distributed=False)  # This works
