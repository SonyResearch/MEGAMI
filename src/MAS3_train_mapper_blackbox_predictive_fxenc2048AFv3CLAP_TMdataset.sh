...
# main config
conf=conf_MAS3_mapper_blackbox_predictive_fxenc2048AFv3CLAP_paired.yaml

n="MAS3_mapper_blackbox_predictive_fxenc2048AFv3CLAP_paired"
...
python M_train_mapper_blackbox_predictive.py --config-name=$conf  \
  ...
  exp.num_workers=8 \
  exp.batch_size=3 \
  dset.validation.num_tracks=6 \
  dset.validation_2.num_tracks=6 \
