...
class Trainer():
    def __init__(self, args=None, dset=None, network=None,  device='cpu', rank=0, world_size=1, distributed=True, tester=None):
        ...
        if self.args.exp.FXenc_args.type=="AFxRep":
            ...
            self.FXenc=fxencode_fn
        elif self.args.exp.FXenc_args.type=="fx_encoder+AF+CLAP":
            Fxencoder_kwargs= self.args.exp.fx_encoder_plusplus_args
            ...
            CLAP_args= self.args.exp.CLAP_args
            from evaluation.feature_extractors import load_CLAP
            CLAP_encoder= load_CLAP(CLAP_args, device=self.device)
            ...
            def fxencode_fn(x):
                ...
                z_CLAP = torch.nn.functional.normalize(z_CLAP, dim=-1, p=2)

                #concatenate z and z_af (rescaling with sqrt(dim) to keep the same scale)

                z=z* math.sqrt(z.shape[-1]) 
                z_af=z_af* math.sqrt(z_af.shape[-1])
                z_CLAP=z_CLAP* math.sqrt(z_CLAP.shape[-1])



                z_all= torch.cat([z, z_af, z_CLAP], dim=-1)
    ...
    def train_step(self):
        ...
        #print("y_pred", y_pred.shape, y_pred.std(), y_pred.mean(), y_pred.min(), y_pred.max())
