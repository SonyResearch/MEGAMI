...
def load_FAC(FAC_args, device, *args, **kwargs):
    from utils.laion_clap.hook import CLAP_Module
    CLAP_args= FAC_args.CLAP_args
    CLAP_model= CLAP_Module(enable_fusion=False, amodel= 'HTSAT-base')
    CLAP_model.load_ckpt(CLAP_args.ckpt_path)
    ...
    def clap_fn(x):
        ...
        with torch.no_grad():
            ...
            emb=CLAP_model.get_audio_embedding_from_data(x,use_tensor=True)

            # Normalize the embeddings
            if CLAP_args.normalize:
                emb = torch.nn.functional.normalize(emb, p=2, dim=-1)

            return emb
    
    Fx_encoder_args= FAC_args.fx_encoder_plusplus_args
    from utils.feature_extractors.fx_encoder_plus_plus import load_model 
    FxEncmodel=load_model(
        model_path=Fx_encoder_args.ckpt_path,
        device=device,
    )
    ...
    def effects_encoder_fn(x):
        ...
        emb=FxEncmodel.fx_encoder(x)
        emb=emb["embedding"]  # Extract the embedding from the dictionary

        if Fx_encoder_args.normalize:
            emb = torch.nn.functional.normalize(emb, p=2, dim=-1)
    ...
    if FAC_args.use_adaptor:
        if FAC_args.adaptor_type == "EffectRemovalMLP":
            from networks.MLP_CLAP_regressor import EffectRemovalMLP
            adaptor=EffectRemovalMLP(dim=2624,hidden_dim=2048)
            ckpt=torch.load(FAC_args.adaptor_checkpoint, map_location=device, weights_only=False)
            adaptor.load_state_dict(ckpt["network"], strict=True)
            adaptor.to(device)

    
    def FAC_fn(x, type=None):
        clap_emb = clap_fn(x)
        clap_emb= clap_emb* math.sqrt(clap_emb.shape[-1])  # Rescale to keep the same scale
        fx_emb = effects_encoder_fn(x)
        fx_emb= fx_emb* math.sqrt(fx_emb.shape[-1])  # Rescale to keep the same scale

        z_af,_ = AFembedding.encode(x)
        z_af= torch.nn.functional.normalize(z_af, p=2, dim=-1)  # Normalize the AF embedding
        z_af= z_af * math.sqrt(z_af.shape[-1])  # Rescale to keep the same scale

        # Concatenate the embeddings
        combined_emb = torch.cat([clap_emb, fx_emb, z_af], dim=-1)

        # Normalize the combined embeddings
        combined_emb = torch.nn.functional.normalize(combined_emb, p=2, dim=-1)

        if type=="wet":
            # If type is wet, apply some specific processing (if needed)
            if FAC_args.use_adaptor:
                combined_emb= adaptor(combined_emb)  # Apply the adaptor if specified
    
        if FAC_args.add_noise:
                combined_emb= torch.nn.functional.normalize(combined_emb, p=2, dim=-1)  # Normalize before adding noise
                combined_emb = add_isotropic_noise(combined_emb, sigma=FAC_args.noise_sigma)


        return combined_emb
        

    return lambda x, type=None: FAC_fn(x, type=type)  # Return a function that takes x and type as arguments
    #load Fx Encoder

def load_fxencAF_adaptor(fxencAF_adaptor_args, device):
    from networks.MLP_CLAP_regressor import MLP_CLAP_regressor
    adaptor_model=MLP_CLAP_regressor(dim=fxencAF_adaptor_args.dim, hidden_dim=fxencAF_adaptor_args.hidden_dim)
    ckpt=torch.load(fxencAF_adaptor_args.ckpt_path, map_location=device, weights_only=False)
    adaptor_model.load_state_dict(ckpt["network"], strict=True)
    adaptor_model.to(device)

    def fxencAF_adaptor_fn(x):
        """
        Apply the adaptor to the input embeddings.
        Args:
            x (Tensor): Input embeddings of shape (B, D)
        Returns:
            Tensor: Adapted embeddings of shape (B, D)
        """
        x= adaptor_model(x)

        if fxencAF_adaptor_args.add_noise:
            x = torch.nn.functional.normalize(x, p=2, dim=-1)  # Normalize before adding noise
            x = add_isotropic_noise(x, sigma=fxencAF_adaptor_args.noise_sigma)
        
        return x
    
    return fxencAF_adaptor_fn
