import math
from utils.ITOMaster_loss import compute_log_rms, compute_crest_factor, compute_stereo_width, compute_stereo_imbalance, compute_log_spread, compute_loudness
import torch
...
class AF_fourier_embedding:
    def __init__(self,
                ...
                output_dim=64,
                sigma=0.2,
                loud_shift=-21.5, #calculated as the mean from the dataset
                loud_scale=4.8, #calculated as the std from the dataset
        ...
        self.loud_shift = loud_shift
        self.loud_scale = loud_scale
    ...
    def encode(self, x):
        ...
        loudness_std, crest_factor_std, log_spread_std, stereo_width_std, stereo_imbalance_std = self.standardize_features(
            loudness, crest_factor, log_spread, stereo_width, stereo_imbalance
        )

        embedding= self.transform(
            loudness_std, crest_factor_std, log_spread_std, stereo_width_std, stereo_imbalance_std
        )


        return embedding, (loudness, crest_factor, log_spread, stereo_width, stereo_imbalance)
    ...
    def standardize_features(self, loudness, crest_factor, log_spread, stereo_width, stereo_imbalance):
        """
        Standardize features using pre-computed mean and std
        """
        loudness = (loudness - self.loud_shift) / self.loud_scale
    ...
    def destandardize_features(self, loudness, crest_factor, log_spread, stereo_width, stereo_imbalance):
        """
        Reverse standardization to get back to original feature space
        """
        #log_rms = log_rms * self.log_rms_scale + self.log_rms_shift
        loudness = loudness * self.loud_scale + self.loud_shift
