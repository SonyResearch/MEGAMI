...
class pSGLD(torch.optim.Optimizer):
    def __init__(self, params, lr=1e-2, alpha=0.99, eps=1e-8, weight_decay=0, add_noise=True):
        """
        Implements preconditioned Stochastic Gradient Langevin Dynamics (pSGLD).

        Args:
            params (iterable): iterable of parameters to optimize or dicts defining parameter groups
            lr (float): learning rate
            alpha (float): smoothing constant for RMSprop (default: 0.99)
            eps (float): term added to denominator to improve numerical stability (default: 1e-8)
            weight_decay (float): optional weight decay (L2 penalty)
            add_noise (bool): whether to add Langevin noise (set False for deterministic behavior)
        """
        defaults = dict(lr=lr, alpha=alpha, eps=eps, weight_decay=weight_decay, add_noise=add_noise)
    ...
    def step(self):
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                precond = 1.0 / (square_avg.sqrt() + eps)

                # Gradient step
                step = 0.5 * lr * precond * grad
                p.data.add_(-step)

                # Langevin noise
                if add_noise:
                    noise_std = math.sqrt(lr) * precond
                    noise = torch.normal(mean=0.0, std=1.0, size=p.data.shape, device=p.data.device)
                    p.data.add_(noise_std * noise)


class pSGLD_2(torch.optim.Optimizer):
...
class NoisyAdam(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.99), eps=1e-8, weight_decay=0, warmup = 0, temperature = 0):
        if not 0.0 <= lr:
            raise ValueError("Invalid learning rate: {}".format(lr))
        if not 0.0 <= eps:
    ...
    def __setstate__(self, state):
        super(NoisyAdam, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if self.temperature > 0:
                    noise_std = math.sqrt(scheduled_lr) / denom  # ← same denom as in step
                    noise = self.temperature * torch.randn_like(p_data_fp32) * noise_std.clamp(max=10.0)  # Avoid division by zero
