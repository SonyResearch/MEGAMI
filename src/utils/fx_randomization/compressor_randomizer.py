...
class CompRandomizer:
    ...
    def __init__(self, 
        ...
        smoother_mode_gain=None,  # Options: "iir", "ballistics"
        range_log_threshold=[-60, 0],  # dB range for log threshold randomization
        range_log_ratio=[-2, 3],  # dB range for log ratio randomization
        range_log_knee=[-4.0, 2.0],  # Knee range for compressor randomization
        range_z_alpha_pre=[-4.0, 4.0], # Z-alpha range for compressor randomization
        range_z_alpha_post=[-4.0, 4.0], # Z-alpha range for compressor randomization
    ...
    def forward(self, x, resample=True):
        ...
        input_rms = 20*torch.log10(torch.sqrt(torch.mean(input**2, dim=-1, keepdim=True)))
