...
class Evaluator:
    ...
    def prepare_feature_extractors(self):
        ...
        def generate_Fx_independent(x, input_type="dry",num_samples=1, T=30, cfg_scale=1.0, Schurn=10):
            N, C, L = x.shape  # B is the batch size, N is the number of tracks, C is the number of channels and L is the length of the audio
            B=1
        
        
            shape=self.sampler.diff_params.default_shape
            shape= [N, 1,*shape[2:]]  # B is the batch size, we want to sample B samples
        
            masks_fwd= torch.ones((B, N), dtype=torch.bool, device=self.device)  # Create masks for all tracks, assuming all tracks are present
            masks_diff= torch.ones((N, 1), dtype=torch.bool, device=self.device)  # Create masks for all tracks, assuming all tracks are present
            ...
            with torch.no_grad():
                ...
                cond =einops.rearrange(cond, 'b n c l -> (b n) 1 c l')  # Rearrange to [B*N, C, L] where B is the batch size and N is the number of tracks

                #cond=cond.expand(shape[0], -1, -1,-1)  # Expand the condition to match the batch size
                preds, noise_init = self.sampler.predict_conditional(shape, cond=cond.contiguous(), cfg_scale=cfg_scale, device=self.device,  masks=masks_diff)

            preds=einops.rearrange(preds, 'n 1 c -> 1 n c')  # Rearrange to [B*N, C, L] where B is the batch size and N is the number of tracks

            return preds

        self.generate_Fx_independent=lambda x, num_samples : generate_Fx_independent(x, input_type="wet", num_samples=num_samples, T=self.method_args.T, cfg_scale=self.method_args.cfg_scale, Schurn=self.method_args.Schurn)
    ...
    def run_evaluation_paired(self):
            ...
            for i in range(len(self.dataset)):
                ...
                #if os.path.exists(f"{path_segment}/{self.experiment_name}/random.wav"):
                #    continue
	

                preds=self.generate_Fx_independent(x_dry, 1) 
                z_pred=self.embedding_post_processing(preds)  # post-process the generated features
                torch.cuda.empty_cache()

                #z_pred=einops.rearrange(z_pred, '(B N) D -> B N D', B=1, N=N)  # Rearrange to [B, N, D] where B is the batch size, N is the number of tracks and D is the dimension of the features (2048 + 2048 = 4096)
