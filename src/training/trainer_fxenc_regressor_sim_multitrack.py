...
class Trainer():
    def __init__(self, args=None, dset=None, val_set_dict=None, network=None,  device='cpu', rank=0, world_size=1, distributed=True):
        ...
        def encode_fn(x):
                ...
                """
                x=x.to(self.device)
                #z_CLAP=CLAP_encoder(x, type="dry") #shape (B, C)
                #z_CLAP=torch.nn.functional.normalize(z_CLAP, dim=-1, p=2)  # normalize to unit variance
                #z_CLAP=z_CLAP*math.sqrt(z_CLAP.shape[-1])  # rescale to keep the same scale

                z=feat_extractor(x.contiguous())  # encode x with fx encoder, shape (B, C, L)
                ...
                z_af=torch.nn.functional.normalize(z_af, dim=-1, p=2)  # normalize to unit variance
                ...
                norm_z= torch.nn.functional.normalize(z_all, dim=-1, p=2)  # normalize to unit variance
    ...
    def train_step(self):
        ...
        with torch.no_grad():
            ...
            #let's do stereo to mono conversion if needed
            if x.shape[1] == 2:
            ...
            x_encoded = self.encode(x.contiguous())  # encode x with CLAP encoder
            y_encoded = self.encode(y.contiguous())  # encode y with CLAP encoder
            ...
            for data in val_loader:
                ...
                with torch.no_grad():
                    try:
                        ...
                        print("Error during effects simulation:", e)
                        continue
                    x=x.view(-1, x.shape[-2], x.shape[-1])  # flatten the batch and the number of tracks, so that we have a tensor of shape [B*N, C, L]
                    y=y.view(-1, y.shape[-2], y.shape[-1])  #
