...
class Trainer():
    ...
    def simulate_effects(self, x, cluster, taxonomy, masks):
        ...
        #if (C0).any():
        #    for k in self.effect_randomizer_C0.keys():
        ...
        #        y[indices_k] = self.effect_randomizer_C0[k].forward(x[indices_k])  # apply the effect randomizer to the selected tracks
        #if (C1).any():
        #    for k in self.effect_randomizer_C1.keys():
        #        indices_k=torch.zeros((x.shape[0], x.shape[1]), dtype=torch.bool, device=x.device)  # initialize a tensor of shape [B, N] with False
        #        for index in indices_C1:
        #            for i, t in enumerate(taxonomy[index]):
        #                if t == k:
        #                    indices_k[index, i] = True
        #        y[indices_k] = self.effect_randomizer_C1[k].forward(x[indices_k])  # apply the effect randomizer to the selected tracks
        #return y
    ...
    def train_step(self):
        ...
        #print("simulate effects took", time.time() - a, "seconds")
        

        error, sigma, x_norm, y = self.diff_params.loss_fn(self.network, sample=y, context=x, ema=self.ema, clusters=cluster, taxonomy=taxonomy, masks=masks, compile=self.args.exp.compile)
        #print("loss fn took", time.time() - a, "seconds")
        ...
        if not torch.isnan(loss).any():
            ...
            #print("backward took", time.time() - a, "seconds")
