...
class Trainer():
    def __init__(self, args=None, dset=None, network=None, diff_params=None, tester=None, device='cpu', rank=0, world_size=1, distributed=True):
        ...
        self.simulate_effects_fn= lambda x, cluster: self.simulate_effects(x, cluster)
    ...
    def get_batch(self):
        ...
        data = next(self.dset)
        sample, cluster = data[0].to(self.device), data[1].to(self.device)


        #self.log_audio(sample_y, "train_wet")
        #self.log_audio(sample_x, "train_dry")
    ...
    def train_step(self):
        ...
        x, cluster = self.get_batch()

        #print("sync", self.rank)
        if self.distributed:
            dist.barrier()

        y=self.simulate_effects_fn(x, cluster)

        error, sigma, x_norm, y = self.diff_params.loss_fn(self.network, sample=y, context=x, ema=self.ema, clusters=cluster)
            ...
            if self.rank == 0:
                ...
                if self.args.logging.log:
                    ...
                    if self.args.logging.num_train_samples_to_log > 0 and self.logged_audio_examples < self.args.logging.num_train_samples_to_log:
                        # Log audio samples
                        self.log_audio(y[0].unsqueeze(0), "train_wet" )
                        self.log_audio(x[0].unsqueeze(0), "train_dry" )
                        if x_norm.std() != x.std():
                            self.log_audio(x_norm[0].unsqueeze(0), "train_dry_norm")
