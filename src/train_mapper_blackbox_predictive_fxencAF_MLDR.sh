...
# main config
conf=conf_mapper_blackbox_predictive_fxencAF_MLDR.yaml

n="mapper_blackbox_predictive_fxencAF_MLDR"
...
python train_mapper_blackbox_predictive.py --config-name=$conf  \
  model_dir=$PATH_EXPERIMENT \
  exp.resume=False \
  exp.compile=False \
  exp.num_workers=8 \
  exp.batch_size=4 \
  dset.validation.num_tracks=20 \
  dset.validation_2.num_tracks=20 \
  logging=base_logging_1C_debug
