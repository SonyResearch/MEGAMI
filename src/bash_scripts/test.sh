...
python test.py --config-name=$conf  \
  ...
  tester.wandb.run_name="cond:dry12s_model:waveformSTFT_traindata:tencymastering_its:200k" \
...
python test.py --config-name=$conf  \
  ...
  tester=evaluate_conditional_dry_vocals \
  tester.checkpoint=$ckpt \
  tester.wandb.run_name="cond:dry12s_model:waveformSTFT_traindata:tencymastering_its:100k" \
...
python test.py --config-name=$conf  \
  ...
  tester=evaluate_conditional_dry_vocals \
  tester.checkpoint=$ckpt \
  tester.wandb.run_name="cond:dry12s_model:waveformSTFT_traindata:tencymastering_its:150k" \
...
python test.py --config-name=$conf  \
  ...
  tester=evaluate_conditional_dry_vocals \
  tester.checkpoint=$ckpt \
  tester.wandb.run_name="cond:dry12s_model:waveformSTFT_traindata:tencymastering_its:50k" \
