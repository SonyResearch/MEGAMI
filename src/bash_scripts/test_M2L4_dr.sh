...
#  tester.wandb.run_name="baseline_fxnorm_mono_dr" \
#  tester.wandb.tags="[\"baseline:fxnorm_mono_dr\"]" \
...
python test.py --config-name=$conf  \
  ...
  dset.validation.num_examples=300 \
  tester.wandb.run_name="cond:fxnorm_mdr_12s_model:M2L4DiT_traindata:TMwetfxnorm_mdr_its:300k_cfg:2" \
  tester.wandb.tags="[\"cond:fxnorm_mdr_12s\",\"model:M2L4DiT\",\"traindata:TMwetfxnorm_mdr\",\"its:300k\",\"cfg:2\"]" \
  tester.wandb.project="project_test_05_06"
