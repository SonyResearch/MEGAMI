...
class Trainer:
    def __init__(
        self, args=None, dset=None, val_set_dict=None, network=None, device="cpu"
    ):
        ...
        self.cossims = (
            []
        )  # This will be used to store the cosine similarities for logging
    ...
    def train_step(self):
        ...
        with torch.no_grad():
            ...
            if x.shape[1] == 2:
                x = x.mean(dim=1, keepdim=True).expand(
                    -1, 2, -1
                )  # expand to [B*N, 1, L] to keep the shape consistent
            elif x.shape[1] == 1:
                x = x.expand(
                    -1, 2, -1
                )  # expand to [B*N, 2, L] to keep the shape consistent

            if y.shape[1] == 2:
                y = y.mean(dim=1, keepdim=True).expand(
                    -1, 2, -1
                )  # expand to [B*N, 1, L] to keep the shape consistent
            elif y.shape[1] == 1:
                y = y.expand(
            ...
            y = apply_RMS_normalization(
                y, -25, x.device
            )  # apply RMS normalization to y

            x_encoded = self.CLAP_encode(x)  # encode x with CLAP encoder
            y_encoded = self.CLAP_encode(y)  # encode y with CLAP encoder

            cossim_base = torch.nn.functional.cosine_similarity(
                x_encoded, y_encoded, dim=1
            )  # cosine similarity between the encoded features
        ...
        cossim = torch.nn.functional.cosine_similarity(
            x_pred, x_encoded, dim=1
        )  # cosine similarity between the predicted and the encoded features
        loss = (
            1 - cossim
        )  # we want to minimize the cosine similarity, so we take 1 - cosine similarity
        ...
        if not torch.isnan(loss).any():
            ...
            if self.args.logging.log:
                ...
                self.cossims.append(cossim.mean().item())
        ...
        for key, val_loader in self.val_set_dict.items():
            ...
            for data in val_loader:
                ...
                with torch.no_grad():
                    ...
                    val_cossim = torch.nn.functional.cosine_similarity(
                        x_pred, x_encoded, dim=1
                    )  # cosine similarity between the predicted and the encoded features
                    val_loss = (
                        1 - val_cossim
                    ...
                    val_loss = torch.nan_to_num(
                        val_loss, nan=0.0, posinf=0.0, neginf=0.0
                    )
            ...
            print(
                f"Validation {key} - Loss: {val_loss_mean}, Cossim: {val_cossim_mean}"
            )
