...
class Trainer():
    ...
    def get_batch(self):
        ...
        if len(data[0]) == 4:
            collated_data = collate_multitrack_paired(data, max_tracks=self.args.exp.max_tracks, sample_rate=self.args.exp.sample_rate, segment_length=self.args.exp.audio_len, device=self.device)  # collate the data, this will pad the data to the maximum number of tracks in the batch
        elif len(data[0]) == 3:
            collated_data = collate_multitrack(data, max_tracks=self.args.exp.max_tracks, sample_rate=self.args.exp.sample_rate, segment_length=self.args.exp.audio_len, device=self.device)  # collate the data, this will pad the data to the maximum number of tracks in the batch
        else:
            raise ValueError("Data must have 3 or 4 elements, got {}".format(len(data[0])))
    ...
    def train_step(self):
        ...
        error, sigma, x_norm, y = self.diff_params.loss_fn(self.network, sample=y, ema=self.ema, clusters=None,  masks=masks)
...
def _main(args):
    ...
    train_loader = torch.utils.data.DataLoader(dataset=train_set, batch_size=args.exp.batch_size,
                                               ...
                                               worker_init_fn=worker_init_fn, timeout=0, prefetch_factor=5, collate_fn=lambda x: x)
    train_loader = iter(train_loader)

    val_set_dict = {}
    val_set = hydra.utils.instantiate(args.dset.validation)
    val_loader = torch.utils.data.DataLoader(dataset=val_set, batch_size=args.exp.val_batch_size,  collate_fn=lambda x: x)
    val_set_dict[args.dset.validation.mode] = val_loader

    #try:
    val_set_2 = hydra.utils.instantiate(args.dset.validation_2)
    val_loader_2 = torch.utils.data.DataLoader(dataset=val_set_2, batch_size=args.exp.val_batch_size,  collate_fn=lambda x: x)
    ...
    trainer = Trainer( args=args, dset=train_loader, network=network, diff_params=diff_params, tester=tester, device=device)  # This works
