...
from testing.tester_blackbox import Tester
import copy
...
class Trainer():
    def __init__(self, args=None, dset=None, network=None,  device='cpu', tester=None):
        ...
        if self.args.exp.loss.type == "MSS_mslr+fxenc":
...
def _main(args):
    ...
    trainer = Trainer( args=args, dset=train_loader, network=network, device=device, tester=tester)  # This works
