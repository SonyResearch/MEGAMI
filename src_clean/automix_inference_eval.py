...
class MusicMixer():
    ...
    def prepare_feature_extractors(self):
        ...
        def apply_effects_full_track(x, z_pred,  seg_size=16384, pad_size=16384):
            ...
            for i in range(x_norm.shape[0]):
                ...
                x_filtered= x_frames_filtered.view(B, C, -1)  # Reshape back to (bs, c, L)
    ...
    def run_automix_full_track(self):
            ...
            for i in range(y_final.shape[0]):
                track_name = os.path.splitext(track_names[i])[0]
                output_path = os.path.join(output_dir, f"{track_name}.wav")
                sf.write(output_path, y_final[i].cpu().numpy().T, 44100)
                print(f"Saved processed track {track_name} to {output_path}")

            #save the final mixture
            mixture_output_path = os.path.join(output_dir, "mixture.wav")
            sf.write(mixture_output_path, y_hat_mixture.cpu().numpy().T, 44100)
            print(f"Saved final mixture to {mixture_output_path}")
...
if __name__ == "__main__":
    ...
    test_set_path="/data4/eloi/test_set/mdx21_dry_stems/mdx21_8instr/stereolized"

    directories = glob.glob(os.path.join(test_set_path, "*"))

    for input_dir in directories:

        #replace stereolized with predicted_random
        output_dir= input_dir.replace("stereolized", "predicted_random")

        os.makedirs(output_dir, exist_ok=True)

        #if os.path.exists(os.path.join(output_dir, "mixture.wav")):
        ...
        try:
            automixer=MusicMixer(method_args=method_args, path_input=input_dir, path_output=output_dir)
    ...
    test_set_path="/data4/eloi/test_set/TM_tracks"

    directories = glob.glob(os.path.join(test_set_path, "*","dry_multi" ))
