...
class MusicMixer():
    def __init__(self, method_args=None, path_input=None,  path_output=None):
        ...
        self.tracks, self.track_names=self.load_data()  # Load the dataset
    ...
    def load_data(self, path_input=None, to_mono=True):
        ...
        audio_files = [f for f in os.listdir(path_input) if f.endswith('.wav') or f.endswith('.mp3') or f.endswith('.flac')]
        if not audio_files:
            raise Exception(f"No audio files found in {path_input}")
        else:
            print(f"Found {len(audio_files)} audio files in {path_input}")
        ...
        for i in range(len(audio_files)):
            file_path = os.path.join(path_input, audio_files[i])
        ...
        tracks= torch.stack(tracks, dim=0)  # shape [N, C, L] where N is the number of tracks, C is the number of channels and L is the length of the audio
    ...
    def prepare_feature_extractors(self):
        ...
        def generate_Fx_conditional(x, x_wet, input_type="dry",num_samples=1, T=30, cfg_scale=1.0, Schurn=10):
            N, C, L = x.shape  # B is the batch size, N is the number of tracks, C is the number of channels and L is the length of the audio
            N_wet, C_wet, L_wet=x_wet.shape

            B=1
        
            shape=self.sampler.diff_params.default_shape
            shape= [num_samples, N+N_wet,*shape[2:]]  # B is the batch size, we want to sample B samples
        
            masks_fwd= torch.ones((B, N), dtype=torch.bool, device=self.device)  # Create masks for all tracks, assuming all tracks are present
            masks_fwd_wet= torch.ones((B, N_wet), dtype=torch.bool, device=self.device)  # Create masks for all wet tracks, assuming all tracks are present

            masks_diff= torch.ones((num_samples, N+N_wet), dtype=torch.bool, device=self.device)  # Create masks for all tracks, assuming all tracks are present
            ...
            def fwd_operator(x_hat):
                """
                x shape Tensor of shape [B, N+Nw, C1, C2]
                y shape Tensor of shape [B, Nw, C1, C2]
                """
            ...
            with torch.no_grad():
                is_wet= "wet" in input_type
                cond, x_preprocessed=self.sampler.diff_params.transform_forward(x.unsqueeze(0),  is_condition=True, is_test=True, masks=masks_fwd, is_wet=is_wet)
                cond_wet, x_wet_preprocessed=self.sampler.diff_params.transform_forward(x_wet.unsqueeze(0),  is_condition=True, is_test=True, masks=masks_fwd_wet, is_wet=True)

                cond=cond.expand(shape[0], -1, -1,-1)  # Expand the condition to match the batch size
                cond_wet=cond_wet.expand(shape[0], -1, -1,-1)  # Expand the condition to match the batch size
    ...
    def run_automix_MST(self, path_ref_track):
            ...
            #make sure there is no overlap between track_names_wet and track_names, if there is, remove the track in x_dry and track_names
            if set(track_names_wet) & set(track_names):
                print(f"Removing {len(set(track_names_wet) & set(track_names))} tracks from the input dry audio that are present in the wet sources")
                #shape before removing tracks is (N, C, L)
                overlap_tracks = [name for name in track_names if name in track_names_wet]
                x_dry = x_dry[[i for i, name in enumerate(track_names) if name not in overlap_tracks]]
                track_names = [name for name in track_names if name not in overlap_tracks]
            ...
            x_wet=x_wet.to(self.device)  # Move wet sources to the same device

            preds=self.generate_Fx_conditional(x_dry, x_wet, 1) 
    ...
    def run_automix_conditional(self, path_wet_sources):
...
if __name__ == "__main__":
    ...
    automixer.run_automix_MST("examples/example_conditional/wet_sources")
