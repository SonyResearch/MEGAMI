...
class Tester():
        ...
        for k, test_set in self.test_set_dict.items():
            ...
            for data  in tqdm(test_set):
                ...
                if input_type == "dry" or input_type == "fxnorm_dry":
                    preds=self.sample_conditional_style_multitrack(mode, x, B=B, N=N,  masks=masks, taxonomy=taxonomy, input_type=input_type)
                elif input_type == "fxnorm_wet":
                    preds=self.sample_conditional_style_multitrack(mode, y, B=B,N=N,  masks=masks, taxonomy=taxonomy, input_type=input_type)
    ...
    def sample_conditional_style_multitrack(self, mode,  cond, B=1, N=3, cluster=None, taxonomy=None, masks=None, input_type="dry"):
