...
class NCSNpp(nn.Module):
    ...
    def forward(self, x, time_cond=None, input_concat_cond=None):
        ...
        #x shape is B, N, C, F, T
        # x is complex

        print("x", x.shape, "t", time_cond.shape)
        B, N, Cy, F, T = x.shape

        x=einops.rearrange(x, "b n c f t -> b (n c) f t")  # b,2*D,F,T

        modules = self.all_modules
        m_idx = 0

        time_cond=time_cond.view(-1)
        ...
        if input_concat_cond is not None:
            input_concat_cond = einops.rearrange(input_concat_cond, "b n c f t -> b (n c) f t")
            x_chans = []
            C= input_concat_cond.shape[1]  # C = 2 * D
            for i in range(C):
                x_i= input_concat_cond[:, i, :, :].unsqueeze(1)  # b,1,F,T
                x_chans.append(torch.cat([x_i.real, x_i.imag], dim=1))  # b,2,F,T
            x_cond = torch.cat(x_chans, dim=1) #4*D

            x_spec_in = torch.cat([x_spec_in, x_cond], dim=1)
        ...
        if return_complex:
            ...
            #x_spec shape is B, 2*D, F, T
            x_spec = einops.rearrange(x_spec, "b (n c k) f t -> b n c f t k", n=N, c=Cy, k=2)  # b,2,D,F,T
            x_spec=x_spec.contiguous()
            x_spec = torch.view_as_complex(x_spec)  # Convert to complex
...
class NCSNppTime(NCSNpp):
    ...
    def istft(self, spec, length=None):
        ...
        sig=einops.rearrange(sig, "(b n c) t -> b n c t", c=c,n=n)
        return sig[..., :length]
    ...
    def forward(self, x, time_cond=None, input_concat_cond=None):
        ...
        assert input_concat_cond.shape[0] == B, "input_concat_cond must have the same batch size as x {} but got {}".format(input_concat_cond.shape[0], B)
        assert input_concat_cond.shape[1] == N, "input_concat_cond must have the same number of channels as x {} but got {}".format(input_concat_cond.shape[1], self.input_channels)
        assert input_concat_cond.shape[-1]== T, "input_concat_cond must have the same time dimension as x {} but got {}".format(input_concat_cond.shape[-1], T)

        x=einops.rearrange(x, "b n c t -> b (n c) t")  # b,2*D,F,T


        input_concat_cond=einops.rearrange(input_concat_cond, "b n c t -> b (n c) t")



        if input_concat_cond is not None:
            input_stft=torch.cat([x, input_concat_cond], dim=1)
            res_stft=self.stft(input_stft)

            x_spec=res_stft[:, :(N*Cy)]
            cond_spec=res_stft[:, (N*Cy):].abs()  # take the absolute value of the condition spec
        else:
            raise ValueError("input_concat_cond must be provided for NCSNppTime")
            x_spec=self.stft(x)
        
        #print("x_spec.shape", torch.isnan(x_spec).any(), x_spec.shape)

        if x_spec.shape[-2] % 16 != 0:
            raise ValueError("x_spec must have a time dimension that is a multiple of 16, but got {}".format(x_spec.shape[-2]))
        ...
        #print("x_spec_in.shape", torch.isnan(x_spec_in).any(), x_spec_in.shape)
        ...
        x_spec = einops.rearrange(x_spec, "b (n c) f t -> b n c f t", n=N, c=Cy*2)
        ...
        x_l= einops.rearrange(x_l, "b n k f t -> b n f t k")  # b,2*D,F,T
        x_l=x_l.contiguous()
        #x_l = torch.permute(x_l, (0,1, 3, 4, 5, 2)).contiguous() # b,2,D,F,T -> b,D,F,T,2
        assert x_l.shape[-1] == 2, "x_l must have a last dimension of size 2, but got {}".format(x_l.shape[-1])

        x_r= einops.rearrange(x_r, "b n k f t -> b n f t k")
        x_r=x_r.contiguous()
        #x_r = torch.permute(x_r, (0,1, 3, 4, 5, 2)).contiguous() # b,N,2,D,F,T -> b,N, D,F,T,2
        assert x_r.shape[-1] == 2, "x_r must have a last dimension of size 2, but got {}".format(x_r.shape[-1])

        x_l = torch.view_as_complex(x_l)  # Convert back to complex
        x_r = torch.view_as_complex(x_r)

        #print("x_l.shape", torch.isnan(x_l).any(), x_l.shape)
        #print("x_r.shape", torch.isnan(x_r).any(), x_r.shape)   

        #print("x_l.shape", x_l.shape, "x_r.shape", x_r.shape)
        x_l=x_l.unsqueeze(2)  # b, N, 1, F, T
        x_r=x_r.unsqueeze(2)  # b, N, 1, F

        x_spec = torch.cat([x_l, x_r], dim=2) # B, N, D, F, T
        ...
        #print("x-time", torch.isnan(x_time).any())
