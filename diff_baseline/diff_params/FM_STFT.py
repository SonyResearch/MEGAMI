...
class FM_STFT(SDE):
    ...
    def loss_fn(self, net, sample=None, context=None, *args, **kwargs):
        ...
        with torch.no_grad():
            ...
            Y=self.transform_forward(y, normalize=False)/self.sigma_data
            ...
            X=self.transform_forward(x, normalize=True)/ self.sigma_data
        ...
        estimate = net(input, t.squeeze(-1))
    ...
    def model_call(self, x, net, t, cond=None,  cfg_scale=1.0, *args, **kwargs):
        ...
        v=net(x, cnoise.to(torch.float32))  #this will crash because of broadcasting problems, debug later!
    ...
    def transform_forward(self, x, normalize=False, **kwargs):
        #TODO: Apply forward transform here
        if normalize:
            B, N, C, T = x.shape
            x= einops.rearrange(x, "b n c t -> (b n) c t")

            x= apply_RMS_normalization(x, RMS_norm=-25, use_gate=False)

            x= einops.rearrange(x, "(b n) c t -> b n c t", b=B, n=N)
